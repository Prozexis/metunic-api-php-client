...
class TransactionsApi
{
     ...
     *
     * @param  string $currency Kredi bilgisi istenen kur. (required)
     *
     * @throws \Swagger\Client\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \Swagger\Client\Model\GeneralWebServiceResponse
     */
    public function getCredits($currency)
    {
        list($response) = $this->getCreditsWithHttpInfo($currency);
        return $response;
    }
     ...
     */
    public function getCreditsWithHttpInfo($currency)
    {
        $returnType = '\Swagger\Client\Model\GeneralWebServiceResponse';
        $request = $this->getCreditsRequest($currency);
     ...
     */
    public function getCreditsAsync($currency)
    {
        return $this->getCreditsAsyncWithHttpInfo($currency)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getCreditsAsyncWithHttpInfo($currency)
    {
        $returnType = '\Swagger\Client\Model\GeneralWebServiceResponse';
        $request = $this->getCreditsRequest($currency);
     ...
     */
    protected function getCreditsRequest($currency)
    {
        // verify the required parameter 'currency' is set
        if ($currency === null || (is_array($currency) && count($currency) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $currency when calling getCredits'
            );
        }

        $resourcePath = '/transactions/credit';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
