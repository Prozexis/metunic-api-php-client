...
class LoginApi
{
     ...
     * Auth
     *
     * @param  string $username Bayi için kullanıcı adı. (required)
     * @param  string $password Bayi için parola. (required)
     *
    ...
    {
        list($response) = $this->authWithHttpInfo($username, $password);
        return $response;
    }
     ...
     */
    public function authWithHttpInfo($username, $password)
    {
        $returnType = '\Swagger\Client\Model\GeneralWebServiceResponse';
        $request = $this->authRequest($username, $password);
     ...
     */
    public function authAsync($username, $password)
    {
        return $this->authAsyncWithHttpInfo($username, $password)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function authAsyncWithHttpInfo($username, $password)
    {
        $returnType = '\Swagger\Client\Model\GeneralWebServiceResponse';
        $request = $this->authRequest($username, $password);
     ...
     */
    protected function authRequest($username, $password)
    {
        // verify the required parameter 'username' is set
        if ($username === null || (is_array($username) && count($username) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $username when calling auth'
            );
        }
        // verify the required parameter 'password' is set
        if ($password === null || (is_array($password) && count($password) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $password when calling auth'
            );
        }

        $resourcePath = '/login/auth';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
